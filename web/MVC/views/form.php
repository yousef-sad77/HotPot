...
{
  $signup = $_SESSION['form_data']['signup'] ?? '';
...
<form class='px-4 pb-2 auth-form' id='signin-form' action='../main/signin.php' method='post'>
  <div class='mb-3'>
    ...
    <div>";
  if (!empty($email_error)) {
    echo "<p style='color: red; font-size: .5rem; font-weight: 100;'>" . htmlspecialchars($email_error) . "</p>";
  }
  echo "  </div>
  </div>
  <div class='mb-3'>
    ...
    <div>";
  if (!empty($pwd_error)) {
    echo "<p style='color: red; font-size: .5rem; font-weight: 100;'>" . htmlspecialchars($pwd_error) . "</p>";
  }
  echo "  </div>
  </div>
</form>";
